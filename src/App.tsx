...
const DOUBLE_TAP_DELAY = 300; // ms threshold for double-tap detection
...
export default function App() {
  ...
  // Keep isFullscreen state in sync with actual browser fullscreen
  useEffect(() => {
  ...
  // Handle tap interactions on the main view area:
  // - NOT in fullscreen: double-tap enters fullscreen
  // - IN fullscreen: single OR double tap exits fullscreen AND enables safe mode
  const handleTap = useCallback(() => {
    const now = Date.now();
    const timeSinceLastTap = now - lastTapRef.current;

    if (isFullscreen) {
      // Any tap while in fullscreen: exit fullscreen and switch to safe mode
    ...
    // Not in fullscreen: double-tap to enter fullscreen
    if (timeSinceLastTap < DOUBLE_TAP_DELAY && timeSinceLastTap > 0) {
      ...
      // First tap — start timer to detect double-tap
      lastTapRef.current = now;
      if (tapTimerRef.current) clearTimeout(tapTimerRef.current);
      tapTimerRef.current = setTimeout(() => {
        tapTimerRef.current = null;
        lastTapRef.current = 0;
        // Single-tap: no-op (could be extended to toggle play)
      }, DOUBLE_TAP_DELAY);
    }
  }, [isFullscreen, isSuggestive]);
  ...
  return (
    ...
    >
      {/* Tap / Double-tap capture overlay — sits above images, below controls */}
      <div
      ...
      >
        ...
        {/* Mode Toggle — above the tap overlay */}
        <button
          onClick={toggleMode}
          className={`flex items-center gap-2 px-4 py-2 rounded-full backdrop-blur-md border transition-all duration-300 z-50 relative ${
            isSuggestive
      ...
      >
        <div className="flex items-center gap-6 mb-6">
          ...
          <button
            ...
            className="p-3 rounded-full bg-white/10 text-white hover:bg-white/20 backdrop-blur-md transition-all hover:scale-110 active:scale-95"
            title={isFullscreen ? 'Exit Fullscreen' : 'Enter Fullscreen (or double-tap)'}
          >
        ...
        {isFullscreen && (
          <div className="mt-1 text-white/30 text-xs">
            Tap to exit fullscreen &amp; safe mode
          </div>
        )}
      </div>
    </div>
